...
(function () {
    //TODO: loader
    var app = angular.module("shortcakes", ["ngSanitize", "ui.bootstrap", "ui.grid", "ui.grid.selection", "ui.select", "ui.grid.autoResize"]);

    app.controller("ShortcakesCtrl", function ($scope, $http, $modal, $filter) {
        var store = this;
        ...
        store.loadList = function (page, limit) {
            ...
            $http.get("/api/shortcakes" + params)
        ...
        store.open = function (recordId) {
            $modal.open({
                animation: true,
                templateUrl: 'js/shortcakes/shortcakes-edit.html',
                controller: 'ShortcakesEditFormCtrl',
                size: "lg",
                resolve: {
                    recordId: function () {
                        return recordId;
        ...
        store.showDelete = function() {
            $modal.open({
                ...
                controller: 'ShortcakesDeleteConfirmCtrl',
                resolve: {
                    recordId: function () {
    ...
    app.directive("shortcakesList", function () {
        return {
            restrict: "E",
            templateUrl: "js/shortcakes/shortcakes-list.html"
        }
    });

    app.controller('ShortcakesDeleteConfirmCtrl', function ($http, $scope, $modalInstance, $timeout, recordId, sender) {
        var store = this;
        ...
        $scope.ok = function () {
            $scope.loading = true;
            $scope.alerts = [];
            $http.delete("/api/shortcakes/" + recordId)
                .success(function () {
                    $modalInstance.close();
    ...
    app.controller('ShortcakesEditFormCtrl', function ($http, $scope, $modalInstance, $timeout, $filter, recordId, sender) {
        var store = this;
        ...
        store.loadRecord = function () {
            $scope.loading = true;
            $http.get("/api/shortcakes/" + recordId)
                .success(function (data) {
        ...
        $scope.ok = function () {
            ...
            $http.post("/api/shortcakes", $scope.formRecord)
                .success(function () {
                    $modalInstance.close();
                    sender.refresh();
                })
