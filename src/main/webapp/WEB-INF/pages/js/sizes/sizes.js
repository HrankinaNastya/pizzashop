...
(function () {
    //TODO: loader
    var app = angular.module("sizes", ["ngSanitize", "ui.bootstrap", "ui.grid", "ui.grid.selection", "ui.select", "ui.grid.autoResize"]);

    app.controller("SizesCtrl", function ($scope, $http, $modal, $filter) {
        var store = this;
        ...
        store.open = function (recordId) {
            $modal.open({
                animation: true,
                templateUrl: 'js/sizes/sizes-edit.html',
                controller: 'SizesEditFormCtrl',
                size: "lg",
                resolve: {
                    recordId: function () {
                        return recordId;
        ...
        store.showDelete = function() {
            $modal.open({
                ...
                controller: 'SizesDeleteConfirmCtrl',
                resolve: {
                    recordId: function () {
    ...
    app.directive("sizesList", function () {
        return {
            restrict: "E",
            templateUrl: "js/sizes/sizes-list.html"
        }
    });

    app.controller('SizesDeleteConfirmCtrl', function ($http, $scope, $modalInstance, $timeout, recordId, sender) {
        var store = this;
        ...
        $scope.ok = function () {
            $scope.loading = true;
            $scope.alerts = [];
            $http.delete("/api/sizes/" + recordId)
                .success(function () {
                    $modalInstance.close();
    ...
    app.controller('SizesEditFormCtrl', function ($http, $scope, $modalInstance, $timeout, $filter, recordId, sender) {
        var store = this;
        ...
        $scope.ok = function () {
            ...
            $http.post("/api/sizes", $scope.formRecord)
                .success(function () {
                    $modalInstance.close();
                    sender.refresh();
                })
